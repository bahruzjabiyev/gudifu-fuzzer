...
void* runServerThread(void*) {
  ...
  std::string conf_template_name = "/src/httpd.conf";
  std::string ext = ".conf";
  ...
  const char *my_argv[] = {"/tmp/httpd", "-f", conf_file_name, "-E", "/tmp/apache.log", "-X", NULL};
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t data_len) {
  ...
  if (std::find(std::begin(cacheable_codes), std::end(cacheable_codes), response_code) != std::end(cacheable_codes)) {
    ...
    writeToFile(response_code, "/4/4.66/logs/response_apache_"+request_sha_str, true);
  }
